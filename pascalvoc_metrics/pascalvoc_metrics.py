...
def main():
    ...
    axis_names = ['Score', 'Recall','Precision']
    df_1 = pd.DataFrame(det_bboxes['Score'])
    df_2 = pd.DataFrame(result['recall'])
    df_3 = pd.DataFrame(result['precision'])
    df = pd.concat([df_1,df_2,df_3],axis=1).sort_values('Score').set_axis(axis_names, axis=1)
    ...
    save_path_2 = args.output / f"score_recall_precision.csv"
